...
def dump_retweets_job(tweet: Tweet, config: Config, twython_connector: TwythonConnector):
    ...
    dump_dir = "{}/{}/{}/{}".format(config.dump_root, tweet.news_source, tweet.label,tweet.news_id)
...
def collect_retweets(news_list, news_source, label, config: Config):
    ...
    for news in news_list:
        for tweet_id in news.tweet_ids:
            try:
                with open("{}/{}/tweets/{}.json".format(save_dir,news.news_id,tweet_id),'r') as f:
                    tweet_j=json.load(f)
                    if tweet_j['retweet_count']>0:
                        tweet_id_list.append(Tweet(tweet_id, news.news_id, news_source, label))
            except Exception as ex:
                print('Something went wrong in retweet_collection lines 49-52')
                print(str(ex))
